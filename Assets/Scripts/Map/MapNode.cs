...
public class MapNode : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler, IPointerExitHandler
{
    ...
    public void SetUp(Node node, NodeBlueprint blueprint)
    {
        ...
        if (blueprint.nodeType == NodeType.MinorEnemy)
        {
            enemyPrefab = EnemyManager.Instance.GetCommonEnemy();
            Enemy enemy = enemyPrefab.GetComponent<Enemy>();

            image.sprite = GameDataReferences.Instance.allEnemySprites[enemy.stars - 1];
        }
        else if (blueprint.nodeType == NodeType.EliteEnemy)
        {
            enemyPrefab = EnemyManager.Instance.GetEliteEnemy();
            Enemy enemy = enemyPrefab.GetComponent<Enemy>();

            image.sprite = GameDataReferences.Instance.allEnemySprites[enemy.stars - 1];
        }
        else if (blueprint.nodeType == NodeType.Boss)
        {
            enemyPrefab = EnemyManager.Instance.GetBossEnemy();
            Enemy enemy = enemyPrefab.GetComponent<Enemy>();
